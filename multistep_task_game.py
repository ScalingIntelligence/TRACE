"""
Realistic Multi-Step Task Game - Training Environment for Multi-Step Completion

Trains the model to handle the *actual* mechanics that cause tau2-bench
multi-step failures: one-shot tool constraints, information retrieval chains,
item batching, status-dependent tool selection, cross-entity referencing,
address propagation, and incremental user requests.

All data is procedurally generated — no tau2-bench prompts, policies, or
database content is used.

Reward: partial credit per correct operation + penalties for one-shot
violations, wrong tool types, and skipped lookups.
"""
...
# =============================================================================
# Constants (all original — no tau2-bench data)
# =============================================================================
...
# Difficulty presets (mirrors structured_data_game pattern)
DIFFICULTY_CONFIG = {
    ...
    3: {"orders": (2, 3), "items_per_order": (2, 3), "ops": (3, 4),
        "templates": ["cross_order_ops", "multi_item_batch", "address_propagation",
                      "status_dependent", "cross_ref_address"]},
    4: {"orders": (3, 4), "items_per_order": (2, 3), "ops": (3, 5),
        "templates": ["cross_order_ops", "multi_item_batch", "address_propagation",
                      "status_dependent", "cross_ref_address", "conditional_exchange",
                      "incremental_reveal"]},
    5: {"orders": (3, 4), "items_per_order": (2, 3), "ops": (4, 5),
        "templates": ["cross_order_ops", "multi_item_batch", "address_propagation",
                      "status_dependent", "cross_ref_address", "conditional_exchange",
                      "incremental_reveal"]},
}
...
# =============================================================================
# Tool schemas (original names, no tau2-bench prompts)
# =============================================================================
...
def _tmpl_conditional_exchange(rng: random.Random, orders: List[OrderEntity],
                               products: List[ProductEntity]) -> Optional[TaskOperation]:
    """Exchange item for cheapest variant; if none cheaper, return it instead."""
    ...
    available = [v for v in product.variants if v.available and v.item_id != item.item_id]
    if not available:
        return None
    ...
    if cheapest.price < item.price:
        # Cheaper variant exists → exchange
        return TaskOperation(
            description=(
                f"exchange the {item.name} ({item.item_id}) in order {order.order_id} "
                f"for the cheapest available variant of the same product. "
                f"If no cheaper variant exists, return it instead for a refund"
            ),
            order_id=order.order_id,
            tool_name="exchange_delivered_order_items",
            tool_args={
                "order_id": order.order_id,
                "item_ids": [item.item_id],
                "new_item_ids": [cheapest.item_id],
                "payment_method_id": order.payment_method_id,
            },
            requires_lookup=True,
        )
    else:
        # No cheaper variant → return
        return TaskOperation(
            description=(
                f"exchange the {item.name} ({item.item_id}) in order {order.order_id} "
                f"for the cheapest available variant of the same product. "
...
def generate_episode(seed: int, difficulty: int = 3) -> EpisodeScenario:
    ...
    for template_name in available_templates:
        ...
        elif template_name == "conditional_exchange":
            free_delivered = [o for o in orders if o.status == "delivered"
                              and o.order_id not in used_orders]
            if free_delivered:
                op = _tmpl_conditional_exchange(rng, free_delivered, products)
                if op and op.order_id not in used_orders:
                    operations.append(op)
                    used_orders[op.order_id] = op.tool_name
...
# =============================================================================
# Tool simulation (stateful, mirrors tau2-bench mechanics)
# =============================================================================
...
def compute_reward(
    ...
    if correct_count == n_ops:
        ...
        reward = (correct_count / n_ops) * 0.8

    # Penalties
    reward -= 0.15 * one_shot_violations
    reward -= 0.10 * wrong_tool_type_count
    reward -= 0.10 * wrong_calls
    reward -= 0.05 * skipped_lookups

    reward = max(-1.0, min(1.0, round(reward, 3)))
...
class RealisticMultiStepGame:
    """Multi-turn task game with realistic tau2-bench-like mechanics.
    ...
    def observe(self, player_id: int) -> str:
        ...
        lines = [
            "# Retail agent policy",
            "",
            "You are a retail customer service agent. You can help users:",
            "- cancel or modify pending orders",
            "- return or exchange delivered orders",
            "- update shipping addresses on pending orders",
            "- look up order and product information",
            "",
            "## Orders",
            "",
            "Each order has a status: **pending**, **delivered**, or other statuses "
            ...
            "",
            "A pending order's items or shipping address can be modified.",
            "Each item can be changed to a different variant of the same product.",
            "Modifying items changes the order status to 'pending (modified)'.",
            "Address changes work on both 'pending' and 'pending (modified)' orders.",
            "",
            "## Return delivered order",
            "",
            "An order can only be returned if its status is 'delivered'.",
            "After return, the order status changes to 'return requested'.",
            "",
            "## Exchange delivered order",
            "",
            "An order can only be exchanged if its status is 'delivered'.",
            "Each item can be exchanged for a different variant of the same product.",
            "After exchange, the order status changes to 'exchange requested'.",
            "",
            "## Products",
            "",
            "Each product has multiple variant items with different options (color, size, material) "
            "and prices. Use get_product_details to see available variants.",
            "",
            "<available_tools>",
            tools_json,
            "</available_tools>",
...
SYSTEM_PROMPT = (
    "You are a retail customer service agent. You can help users cancel or modify "
    "pending orders, return or exchange delivered orders, and update shipping addresses.\n"
    "You should respond with a single JSON tool call at each step.\n"
)
