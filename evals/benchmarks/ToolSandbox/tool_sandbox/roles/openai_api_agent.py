...
class OpenAIAPIAgent(BaseRole):
    ...
    def respond(self, ending_index: Optional[int] = None) -> None:
        ...
        # NOTE: vLLM may return tool_calls=[] (empty list) instead of None
        # when the model generates text without tool calls. We treat both
        # None and empty list as "no tool calls".
...
class VLLMAgent(OpenAIAPIAgent):
    """Agent for any model served via vLLM (OpenAI-compatible API)."""
    model_name: str

    def __init__(self, model_name: str = "Qwen/Qwen3-30B-A3B-Instruct-2507") -> None:
        import os
        self.model_name = os.environ.get("VLLM_AGENT_MODEL", model_name)
