...
class OpenAIServingModels:
    ...
    async def create_weighted_lora(
        ...
        """Create a merged LoRA adapter from a weighted combination of
        registered adapters using exact concat merge.

        The merge computes:
            A_cat = cat([A_1, A_2, ...], dim=0)     # [K*r, in_dim]
            B_cat = cat([w_1*B_1, w_2*B_2, ...], dim=1)  # [out_dim, K*r]

        So that B_cat @ A_cat = sum_i w_i * B_i @ A_i  (exact, no approx).

        The merged adapter is registered as a normal LoRA with rank K*r
        and lora_alpha = K*r (so scaling = 1.0).
        """
        ...
        # Deterministic output name from sorted (name, weight) pairs
        sorted_adapters = sorted(request.adapters, key=lambda a: a.name)
        if request.output_name:
            output_name = request.output_name
        else:
            key_str = "|".join(
                f"{a.name}:{a.weight:.6f}" for a in sorted_adapters
            )
            output_name = "__weighted_merge__"

        # Check if already registered with same name — use load_inplace
        # to overwrite with potentially new weights
        logger.info(
            "Creating weighted LoRA merge: %s",
            [(a.name, a.weight) for a in sorted_adapters],
        )

        # Open safetensors files memory-mapped (no bulk load)
        handles = {}
        adapter_configs = {}
        for wa in sorted_adapters:
            lora_path = self.lora_requests[wa.name].lora_path
            safetensors_path = os.path.join(
                lora_path, "adapter_model.safetensors"
            )
            if not os.path.exists(safetensors_path):
                return create_error_response(
                    message=f"Safetensors file not found for adapter "
                    f"'{wa.name}' at {safetensors_path}",
                    err_type="NotFoundError",
                    status_code=HTTPStatus.NOT_FOUND,
                )
            handles[wa.name] = safe_open(
                safetensors_path, framework="pt", device="cpu"
            )
            # Read adapter config for metadata
            config_path = os.path.join(lora_path, "adapter_config.json")
            if os.path.exists(config_path):
                with open(config_path) as f:
                    adapter_configs[wa.name] = json.load(f)

        # Get reference config from first adapter
        first_config = adapter_configs.get(sorted_adapters[0].name, {})
        base_rank = first_config.get("r", 16)
        num_lora_adapters = len(sorted_adapters)
        merged_rank = base_rank * num_lora_adapters

        # Exact concat merge: iterate module by module
        first_handle = handles[sorted_adapters[0].name]
        merged_tensors = {}

        for tensor_name in first_handle.keys():
            is_lora_a = "lora_A" in tensor_name

            if is_lora_a:
                # A: cat along dim=0 (rank dim), unscaled
                blocks = [
                    handles[a.name].get_tensor(tensor_name)
                    for a in sorted_adapters
                ]
                merged_tensors[tensor_name] = torch.cat(blocks, dim=0)
            else:
                # B: cat along dim=1 (rank dim), weighted
                blocks = [
                    a.weight * handles[a.name].get_tensor(tensor_name)
                    for a in sorted_adapters
                ]
                merged_tensors[tensor_name] = torch.cat(blocks, dim=1)

        # Close handles
        handles.clear()

        # Write merged adapter to temp directory
        merge_dir = os.path.join(
            tempfile.gettempdir(), "vllm_weighted_lora", output_name
        )
        os.makedirs(merge_dir, exist_ok=True)

        # Save merged safetensors
        save_file(
            merged_tensors, os.path.join(merge_dir, "adapter_model.safetensors")
        )

        # Write adapter_config.json with merged rank
        merged_config = dict(first_config)
        merged_config["r"] = merged_rank
        merged_config["lora_alpha"] = merged_rank  # scaling = 1.0
        with open(os.path.join(merge_dir, "adapter_config.json"), "w") as f:
            json.dump(merged_config, f, indent=2)

        # Free merged tensors
        del merged_tensors

        # Register via existing load_lora_adapter path (with load_inplace)
        load_request = LoadLoRAAdapterRequest(
            lora_name=output_name,
            lora_path=merge_dir,
            load_inplace=True,
        )
        result = await self.load_lora_adapter(load_request)
        if isinstance(result, ErrorResponse):
            return result

        logger.info(
            "Weighted LoRA merge created: '%s' (rank %d from %d adapters)",
            output_name, merged_rank, num_lora_adapters,
        )
        return (
            f"Success: Weighted LoRA '{output_name}' created "
            f"(rank={merged_rank}, adapters={num_lora_adapters})."
        )
