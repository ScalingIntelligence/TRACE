...
class LogitsProcessorWithLoRA(BaseLayerWithLoRA):
    ...
    def _get_logits(
        ...
        if self.punica_wrapper.weighted_lora_config:
            # Weighted multi-adapter: deterministic torch.matmul path
            # Avoids Triton lora_shrink which uses SPLIT_K>1 atomic_add
            self.punica_wrapper.add_weighted_lora_logits(
                logits, hidden_states,
                self.lora_a_stacked, self.lora_b_stacked, 1.0,
            )
        else:
            # Normal single-adapter: stock Triton punica path
            lora_output: torch.Tensor | None = (
