...
class FusedMoEWithLoRA(BaseLayerWithLoRA):
    ...
    def _inject_lora_into_fused_moe(self):
        ...
        def act_decorator(layer, func):
            def wrapper(*args, **kwargs):
                ...
                # Weighted multi-adapter: deterministic torch.bmm path
                if self.punica_wrapper.weighted_lora_config:
                    self._weighted_w13_lora(
                        input, hidden_states, curr_topk_ids,
                        topk_weights, expert_map, top_k,
                    )
                    result = func(*args, **kwargs)
        ...
        def moe_sum_decorator(layer, func):
            def wrapper(*args, **kwargs):
                ...
                if self.punica_wrapper.weighted_lora_config:
                    ...
                    curr_topk_ids = moe_state_dict["topk_ids"]
                    expert_map = moe_state_dict["expert_map"]
                    self._weighted_w2_lora(
                        intermediate_cache3, intermediate_cache2,
                        curr_topk_ids, topk_weights, expert_map, top_k,
                    )
                    result = func(*args, **kwargs)
                    return result
    ...
    def _get_lora_config(self):
        """Get (slot, weight) pairs. Set in update_metadata before compile."""
        # weighted_lora_config is None for base model requests
        return self.punica_wrapper.weighted_lora_config or []
    ...
    def _weighted_w13_lora(
        ...
        # Expand hidden_states: [N, hidden]
        x_flat = hidden_states.unsqueeze(1).expand(
            -1, top_k, -1
        ).reshape(N, -1)
