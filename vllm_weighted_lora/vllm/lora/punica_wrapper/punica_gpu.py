...
class PunicaWrapperGPU(PunicaWrapperBase):
    ...
    def update_metadata(
        ...
        **kwargs,
        ...
        # Load weighted LoRA config if present
        self._load_weighted_lora_config(lora_index_to_id)
    ...
    @staticmethod
    def _weighted_cfg_path() -> str:
        port_tag = os.environ.get("VLLM_WEIGHTED_LORA_PORT", "default")
        return os.path.join(
            "/tmp", f"vllm_weighted_lora_config_{port_tag}.json"
        )

    def _load_weighted_lora_config(
        self, lora_index_to_id: list[int | None]
    ):
        """Set weighted LoRA config from file.

        Called in update_metadata BEFORE torch.compile, so .item() is safe.
        Uses mtime-based caching so we only re-read the file when it changes,
        avoiding file I/O on every forward pass.
        """
        # Check if current request has a LoRA active
        active_slot = self._token_lora_indices[0].item()
        if active_slot < 0:
            self.weighted_lora_config = None
            return

        # Read config file only when mtime changes
        raw_config = self._read_weighted_cfg_cached()

        if raw_config:
            # Resolve lora_int_ids to slot indices
            id_to_slot = {
                lid: idx for idx, lid in enumerate(lora_index_to_id)
                if lid is not None
            }
            slot_to_id = {
                idx: lid for idx, lid in enumerate(lora_index_to_id)
                if lid is not None
            }
            active_lora_id = slot_to_id.get(active_slot)
            # Only use the weighted config if the active adapter is part
            # of the weighted set. Otherwise this is a normal single-adapter
            # request and the config file is stale / for a different request.
    ...
    def _read_weighted_cfg_cached(self) -> list | None:
        ...
        The mtime cache was removed because:
        1. Rapid writes (clear → set) can share the same mtime,
           returning stale adapter IDs from a previous conversation.
        2. _load_weighted_lora_config skips file reads when active_slot < 0,
           so the cache never sees file deletions between conversations.
        The file is < 200 bytes; re-reading every forward pass is negligible.
        """
        try:
            with open(self._weighted_cfg_path()) as f:
                raw = json.load(f)
        except (json.JSONDecodeError, OSError):
            return None
        return raw if raw else None

    def add_weighted_lora_linear(
        self,
        y: torch.Tensor,
        x: torch.Tensor,
        lora_a_stacked: tuple[torch.Tensor, ...],
