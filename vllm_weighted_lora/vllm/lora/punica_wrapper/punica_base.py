...
class PunicaWrapperBase(PunicaWrapperABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Weighted LoRA config: list of (slot_index, weight) pairs
        # When set, apply() loops over adapters instead of using a merged one
        self.weighted_lora_config: list[tuple[int, float]] | None = None
        # Cache for config file reads (avoid file I/O every forward pass)
        self._weighted_cfg_mtime: float = 0.0
        self._weighted_cfg_raw: list | None = None
