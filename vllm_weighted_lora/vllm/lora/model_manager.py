...
class LoRAModelManager:
    ...
    def activate_adapter(
        ...
        if lora_id in self._active_adapters:
            return False
        # When VLLM_WEIGHTED_LORA_PIN_SLOT0 is set, the orchestrator is
        # running single-adapter-per-conversation mode.  Deactivate all
        # other adapters so the new one always lands in slot 0 — CUDA
        # graphs bake in lora_a_stacked[slot] memory addresses, so the
        # slot must be deterministic across replays.
        # Guard: only evict when no free slots remain. During CUDA graph
        # warmup, all slots are free so this naturally skips the eviction.
        if os.environ.get("VLLM_WEIGHTED_LORA_PIN_SLOT0"):
            free_slots = sum(1 for s in self.lora_index_to_id if s is None)
            if free_slots == 0:
                for other_id in list(self.lora_index_to_id):
                    if other_id is not None and other_id != lora_id:
                        self._deactivate_adapter(other_id)
                        self._active_adapters.pop(other_id, None)
