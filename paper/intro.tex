...
(ii) \emph{Capability-targeted environment synthesis}: for each identified deficit, an LLM-based generation agent constructs a synthetic environment that isolates the missing behavior while preserving the target environment’s interface—such as its tool schemas, interaction protocol, and output format—so that training remains aligned with the behaviors required at evaluation time. Task instances are procedurally generated from random seeds, and the environment is built so that correct execution of the target capability can be checked automatically from tool arguments, state changes, or final outputs. Each synthesized environment is constructed so that trajectory success depends primarily on the target capability, yielding more focused and attributable supervision than the original environment.
