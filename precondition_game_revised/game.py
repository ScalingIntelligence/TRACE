...
class PreconditionGame:
    ...
    Uses the EXACT same tau2-bench infrastructure for both domains:
    - Same tools (via ToolExecutor wrapping tau2-bench environments)
    - Same policy (airline/retail policy.md)
    - Same message format
    ...
    # -----------------------------------------------------------------
    # Domain-aware policy and tools
    # -----------------------------------------------------------------

    def _get_policy(self) -> str:
        """Return the policy text for the current scenario's domain."""
        if self._scenario and self._scenario.domain == "retail":
            return RETAIL_POLICY
        return AIRLINE_POLICY

    def _get_tool_schemas(self) -> List[Dict[str, Any]]:
        """Return tool schemas for the current scenario's domain."""
        if self._scenario and self._scenario.domain == "retail":
