...
(Hangoo's version with improved reward shaping for better learning signal)
...
Reward structure (IMPROVED with partial credit):
- Partial credit for information gathering progress (up to +0.3)
- Bonus for attempting to respond (+0.1)
- Bonus for correct answer (+0.4)
- Bonus for proper termination timing (+0.2)
- Graduated penalties instead of all-or-nothing -1.0
"""
...
class DependencyResolutionGameHangoo:
    """
    Conversation Termination Game (Hangoo's version with improved rewards).
    ...
    Key improvement: Uses reward shaping with partial credit instead of
    sparse all-or-nothing rewards to provide better learning signal.
    """
    ...
    def _get_required_fields(self) -> Set[Tuple[str, str]]:
        """Get required fields, handling conditional formulas."""
        state = self._state
        gt = state.ground_truth
        ...
        if isinstance(gt.formula, Conditional):
            context = {f"{r}.{f}": state.field_values.get((r, f))
                      for r, f in state.acquired_info}
            return gt.formula.get_full_dependencies(context)
        else:
            return gt.required_fields

    def _finalize(self, phase_at_termination: Optional[ConversationPhase] = None):
        """Compute final reward with partial credit for better learning signal.

        REWARD STRUCTURE (Hangoo's improved version):
        =============================================
        
        1. Information Gathering (up to +0.3):
           - Partial credit for each required field acquired
           - gathering_progress = acquired_required / total_required
           - reward += 0.3 * gathering_progress
        
        2. Response Attempt (+0.1 or -0.3):
           - +0.1 for attempting to respond (even if wrong)
           - -0.3 for never responding at all
        
        3. Answer Correctness (+0.4 or -0.2):
           - +0.4 for correct answer
           - -0.2 for wrong answer (but at least tried)
        
        4. Termination Timing (+0.2, 0, or penalty):
           - +0.2 for ending when user is satisfied (perfect)
           - -0.1 for ending during follow-up (premature)
           - -0.05 per extra loop turn after satisfaction (capped at -0.2)
        
        5. Efficiency Bonus:
           - -0.02 per turn of over-gathering after sufficiency
        
        Example rewards:
        - Perfect execution: 0.3 + 0.1 + 0.4 + 0.2 = 1.0
        - Correct but looped 3 turns: 0.3 + 0.1 + 0.4 + 0 - 0.15 = 0.65
        - Gathered all, wrong answer: 0.3 + 0.1 - 0.2 = 0.2
        - Gathered 50%, wrong answer: 0.15 + 0.1 - 0.2 = 0.05
        - Never responded, gathered 50%: 0.15 - 0.3 = -0.15
        - Never responded, gathered 0: 0 - 0.3 = -0.3
        ...
        # ========== 1. PARTIAL CREDIT FOR INFORMATION GATHERING ==========
        required = self._get_required_fields()
        
        if required:
            acquired_required = state.acquired_info & required
            gathering_progress = len(acquired_required) / len(required)
            reward += 0.3 * gathering_progress
        
        # ========== 2. RESPONSE ATTEMPT CREDIT ==========
        if not state.agent_responses:
            # Never responded - penalize but not as harshly as before
            reward -= 0.3
            # Clamp and return early
            self.rewards = {0: max(-1.0, min(1.0, reward))}
            return
        else:
            # At least tried to respond
            reward += 0.1
        ...
        if answer_correct:
            ...
            # Wrong answer but at least tried
            reward -= 0.2
        ...
        if answer_correct:
            # Only consider timing if answer was correct
            if check_phase == ConversationPhase.USER_FOLLOWUP:
                # Ended when user had follow-up - premature termination
                # Less harsh than original -1.0
                reward -= 0.1
            elif check_phase == ConversationPhase.USER_SATISFIED:
                # Perfect timing - ended when user was satisfied
                reward += 0.2
            elif check_phase == ConversationPhase.LOOP:
                # Looped but eventually ended
                if state.user_satisfied_turn is not None and state.ended_turn is not None:
                    turns_after = state.ended_turn - state.user_satisfied_turn
                    # -0.05 per extra turn, capped at -0.2
                    loop_penalty = min(0.2, 0.05 * max(0, turns_after - 1))
                    reward -= loop_penalty
            # GATHERING phase with correct answer - neutral (didn't get to user response)
        
        # ========== 5. EFFICIENCY BONUS ==========
        if state.sufficiency_turn is not None and state.response_turns:
            first_response_turn = state.response_turns[0]
            over_gathering = max(0, first_response_turn - state.sufficiency_turn - 1)
            # Smaller penalty than original
            reward -= 0.02 * over_gathering
    ...
    def step(self, action: Optional[str]):
        ...
        if action is None:
            self.done = True
            state.done = True
            # Use shaped reward instead of flat -1.0
            self._finalize_on_invalid()
            return
        ...
        if state.current_turn >= self.max_steps and not self.done:
            self._finalize_on_max_steps()

    def _finalize_on_invalid(self):
        """Handle invalid action with shaped reward."""
        state = self._state
        self.done = True
        state.done = True
        self.invalid_player = 0
        
        # Still give partial credit for information gathered
        reward = 0.0
        required = self._get_required_fields()
    ...
    def _finalize_on_max_steps(self):
        """Handle max steps timeout with shaped reward."""
        state = self._state
        self.done = True
        state.done = True
        ...
        # Partial credit for information gathering
        required = self._get_required_fields()
        if required:
            acquired_required = state.acquired_info & required
            gathering_progress = len(acquired_required) / len(required)
        ...
        if not state.agent_responses:
            ...
            if answer_correct:
                reward += 0.4
                # But penalize for not ending properly
                if state.user_satisfied_turn is not None:
                    # Had chance to end but looped until max_steps
                    reward -= 0.2
            else:
                reward -= 0.2
...
if __name__ == "__main__":
    print("Testing Conversation Termination Game (Hangoo's version)")
    print("=" * 60)

    game = DependencyResolutionGameHangoo(min_complexity=1, max_complexity=3)
    ...
    print("\n" + "=" * 60)
    print("TESTING REWARD SCENARIOS")
    print("=" * 60)
    
    # Scenario 1: Gather 0 fields, no response, max steps
    game.reset(100)
    for _ in range(20):
        if not game.done:
            game.step('{"name": "list_records", "arguments": {"record_type": "config"}}')
    print(f"Scenario 1 (no progress, no response): reward = {game.rewards.get(0):.2f}")
    
    # Scenario 2: Gather some fields, no response
    game.reset(101)
    state = game._state
    game.step(f'{{"name": "search_user", "arguments": {{"name": "{state.user_name}"}}}}')
    game.step(f'{{"name": "get_record", "arguments": {{"record_id": "{state.order_id}"}}}}')
    for _ in range(18):
        if not game.done:
            game.step('{"name": "list_records", "arguments": {"record_type": "config"}}')
    print(f"Scenario 2 (gathered fields, no response): reward = {game.rewards.get(0):.2f}")
    
    # Scenario 3: Gather fields, wrong answer
    game.reset(102)
    state = game._state
    game.step(f'{{"name": "search_user", "arguments": {{"name": "{state.user_name}"}}}}')
    game.step(f'{{"name": "get_record", "arguments": {{"record_id": "{state.order_id}"}}}}')
    game.step('{"name": "respond_to_user", "arguments": {"message": "Your total is $999"}}')
    game.step('{"name": "end_conversation", "arguments": {"closing_message": "Goodbye!"}}')
    print(f"Scenario 3 (gathered fields, wrong answer): reward = {game.rewards.get(0):.2f}")
    
    # Scenario 4: Perfect execution
    game.reset(103)
    state = game._state
    gt = state.ground_truth
    game.step(f'{{"name": "search_user", "arguments": {{"name": "{state.user_name}"}}}}')
    game.step(f'{{"name": "get_record", "arguments": {{"record_id": "{state.order_id}"}}}}')
    game.step(f'{{"name": "respond_to_user", "arguments": {{"message": "Your total is ${gt.correct_answer}"}}}}')
    if gt.has_followup:
        game.step('{"name": "respond_to_user", "arguments": {"message": "Here is the additional info."}}')
    game.step('{"name": "end_conversation", "arguments": {"closing_message": "Goodbye!"}}')
    print(f"Scenario 4 (perfect execution): reward = {game.rewards.get(0):.2f}")
