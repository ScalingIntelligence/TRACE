"""Main game class for the precondition verification microenvironment.

Single-player game (player 0 = agent) with LLM user simulator.
Uses the EXACT same airline tools, DB, policy, and message format as tau2-bench.
Tests whether the model checks policy preconditions before acting.
...
from .scenarios import PreconditionScenario, generate_scenario
...
class PreconditionGame:
    """Precondition verification microenvironment.

    Tests the model's ability to check policy rules before acting.
    Uses the EXACT same tau2-bench airline infrastructure:
    - Same tools (via ToolExecutor wrapping tau2-bench AirlineTools)
    - Same policy (airline policy.md)
    - Same message format
    - Same verification (DB hash + write detection)

    Each scenario tests one policy rule with a clear ALLOW/REFUSE outcome.
    ...
    def __init__(
        ...
        # Internal state
        self._scenario: Optional[PreconditionScenario] = None
    ...
    def reset(self, seed: int) -> None:
        ...
        if self._user_client is None:
            raise ValueError(
                "PreconditionGame requires a UserLLMClient. "
    ...
    def get_system_prompt(self) -> str:
        """Return full system prompt with airline policy (matches tau2-bench)."""
        return (
            "<instructions>\n"
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        """Return OpenAI-format tool schemas for airline domain."""
    ...
    def _finalize_with_verification(self) -> None:
        """End game and compute reward via precondition verification."""
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_id": sc.task_id if sc else -1,
        }
