...
class SimpleNegotiationEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state = ta.TwoPlayerState(
            num_players=2,
            seed=seed, max_turns=self.max_turns
    ...
    def get_valid_actions(self, player_id: int) -> list[str]:
        ...
        Returns:
            List of valid action strings: offers, accept/deny, and chat messages
        """
        import random
        
        valid_actions = []
        
        # Check if there's a pending offer to this player
        current_offer = self.state.game_state.get("current_offer")
        if current_offer and current_offer.get("to_player") == player_id:
            # Can accept or deny the offer
        ...
        if player_resources and len(player_resources) >= 2:
            resource_names = list(player_resources.keys())
            
            # Generate a few sample offers
            for _ in range(3):
                offer_resource = random.choice(resource_names)
                request_resource = random.choice([r for r in resource_names if r != offer_resource])
                
                max_offer = min(3, player_resources[offer_resource])
                if max_offer > 0:
