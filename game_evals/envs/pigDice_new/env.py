...
class PigDiceEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None) -> None:
        ...
        self.state.reset(
            game_state=game_state, player_prompt_function=self._player_prompt
        )
    ...
    def get_valid_actions(self, player_id: int) -> list[str]:
        ...
        Returns:
            List of valid action strings: ["[roll]", "[hold]"]
        """
        return ["[roll]", "[hold]"]
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if not match:
            self.state.set_invalid_move(
                reason=f"Invalid action format. Use '[roll]' or '[hold]' or one letter shorthands.",
            )
