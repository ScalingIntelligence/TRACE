...
class KuhnPokerEnv(ta.Env):
    ...
    def get_valid_actions(self, player_id: int) -> List[str]:
        ...
        Returns:
            List of valid action strings: ["[Check]", "[Bet]", ...] depending on game state
        """
        legal_actions = self.state.game_state["current_legal_action_tree"].keys()
        return [f"[{action.capitalize()}]" for action in legal_actions]
