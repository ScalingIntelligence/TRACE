...
- **`{EVAL_RESULTS}`** — Path(s) to your evaluation result file(s). Each file should contain
  trajectories with rewards — both successes (reward = 1.0) and failures (reward < 1.0).
  Can be one file or multiple.
  *Example:* `results/eval_baseline.json`

- **`{MODEL_NAME}`** — Name of the model being evaluated. This is just for record-keeping
  so you know which model's weaknesses you're analyzing.
  *Example:* `Qwen/Qwen3-30B-A3B-Instruct-2507`

**Optional (have sensible defaults that match the paper):**

- **`{N_RUNS}`** — Number of independent labeling runs in Phase 2 for robustness.
  *Default:* `10` (matches the paper's experiments)

- **`{N_CANDIDATES}`** — Max number of candidate capabilities the agent proposes in Phase 1.
  *Default:* `10`

- **`{RHO}`** — Coverage threshold. A capability must be LACKING in at least `{RHO}` fraction
  of the failed trajectories to be considered. *Default:* `0.10` (paper)

- **`{DELTA}`** — Contrastive gap threshold. A capability's LACKING rate on failed
  trajectories must exceed its LACKING rate on successful trajectories by at least `{DELTA}`.
  *Default:* `0.20` (paper)

- **`{K_CONSISTENCY}`** — Cross-run consistency threshold. A capability must pass both
  `{RHO}` and `{DELTA}` in at least `{K_CONSISTENCY}` of `{N_RUNS}` runs to be selected.
  *Default:* `8` (out of 10)

- **`{OUTPUT_DIR}`** — Directory to save all outputs. *Default:* `pipeline/`

### What your evaluation results should look like

Your eval results file should contain trajectories where each one has:
- A **task ID** (unique identifier for the task)
- A **reward** (1.0 for success, < 1.0 for failure)
- The **trajectory itself** — what the agent was asked to do, what actions it took,
  what tools it called, and how it responded

The agent will read through these to label each (trajectory, capability) pair. The exact
JSON schema doesn't matter as long as the agent can read and understand it.

Common formats that work:
- tau2-bench simulation output (has `simulations[].reward_info.reward` and `simulations[].messages`)
- Any JSON with a list of episodes, each containing a reward and conversation/action history
