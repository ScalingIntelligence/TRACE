# TRACE Capability Selection Pipeline (Test Run — Qwen3-8B)
...
- **Model:** `Qwen/Qwen3-8B`
- **Eval results (airline):** `evals/benchmarks/tau2_bench_eval/data/simulations/qwen-3-30b-airline-qwen3-30b.json`
- **Eval results (retail):** `evals/benchmarks/tau2_bench_eval/data/simulations/qwen-3-30b-retail-qwen3-30b.json`
- **Number of Phase 2 runs:** `10`
- **Candidates (Phase 1):** `10`
- **Coverage threshold (`ρ`):** `0.10` (paper default)
- **Contrastive gap threshold (`δ`):** `0.20` (paper default)
- **Cross-run consistency (`K`):** `8` of `10` runs
...
exists with the required packages. The capability selection pipeline only needs
...
# If it does NOT exist, create it and install the required packages
if ! conda env list | grep -qE "^trace\s"; then
  conda create -n trace python=3.11 -y
  conda run -n trace pip install numpy matplotlib
fi
...
If the env already exists, just activate it and verify the packages are installed:

```bash
conda activate trace
python -c "import numpy, matplotlib" || pip install numpy matplotlib
```
...
1. **Phase 1 — Discovery (1 run):** An LLM agent reads all trajectories and proposes up to
   10 candidate capabilities, each with a name, description, and example
   trajectories. Discovery runs **once** to fix the candidate set.

2. **Phase 2 — Labeling (10 parallel subagents):** Given the fixed candidates, each
...
3. **Step 3 — Aggregation:** A small Python script computes coverage `Cov(c)` and contrastive
   gap `Δ(c)` per run, applies the dual-threshold filter `(Cov ≥ 0.10, Δ ≥ 0.20)`,
   and keeps only capabilities that pass in at least `8` of `10` runs.
...
called Qwen/Qwen3-8B that was tested on a set of tasks. Each trajectory is either a
success (reward = 1.0) or a failure (reward < 1.0).

The evaluation results are located at:
- Airline: evals/benchmarks/tau2_bench_eval/data/simulations/qwen-3-30b-airline-qwen3-30b.json
- Retail: evals/benchmarks/tau2_bench_eval/data/simulations/qwen-3-30b-retail-qwen3-30b.json
...
2. **Propose up to 10 candidate capabilities.** Think about capabilities
   in terms of:
...
- Aim for 4-10 meaningful, distinct capability categories
...
2. Save the output to `pipeline/candidate_capabilities.json`
...
## Phase 2: Labeling (10 parallel subagents)

Given the fixed candidates from Phase 1, run 10 independent labeling runs in
...
- **Parallelism:** All 10 runs finish in roughly the time of one.
...
model called Qwen/Qwen3-8B and a fixed set of candidate capabilities. Your job is to label
...
The orchestrating agent should spawn 10 subagents in parallel — all in a single
message so they execute concurrently — using the `Agent` tool.

For each subagent `i` from 1 to 10:
...
Once all 10 subagents complete, run `aggregate_capabilities.py` to compute the
...
```
Cov_r(c) ≥ 0.10   AND   Δ_r(c) ≥ 0.20
```

A capability is **selected** iff it passes in at least `8` of the `10` runs.
...
The final output is `pipeline/selected_capabilities.json`:
...
- **`passes_consistency`** — Whether `n_runs_passed ≥ 8`
...
This file is the input to the next step: [Environment Generation](./test_environment_generation.md).
...
```
Eval results (airline + retail simulations for Qwen/Qwen3-8B)
        │
        ▼
┌──────────────────────────────────────┐
│  Phase 1: Discovery (1 run)          │  Agent reads trajectories, proposes
│  Produces candidate_capabilities.json│  up to 10 candidates with
│                                      │  names + descriptions + examples
└──────────┬───────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│  Phase 2: Labeling (×10)             │  Parallel subagents. Each labels
│  Three-way labels:                   │  every (trajectory, capability)
│  NA / PRESENT / LACKING              │  pair. Fresh isolated contexts.
│  Produces run_01..run_10.json        │
└──────────┬───────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│  Step 3: aggregate_capabilities.py   │  Per-run: Cov(c), Δ(c)
│  Filter: Cov ≥ 0.10 AND Δ ≥ 0.20    │  Cross-run: must pass in
│  Cross-run consistency: ≥ 8/10       │  ≥ 8 of 10 runs
└──────────┬───────────────────────────┘
           │
           ▼
   ...
   (with Cov, Δ, descriptions, examples, PENDING status)
           │
           ▼
   Next: test_environment_generation.md
```
...
- **`docs/contrastive_skill_selection_prompt.md`** — Earlier reference prompt (tau2-bench specific).
- **`evals/benchmarks/tau2_bench_eval/contrastive_skill_selection_llm.py`** — Earlier reference: programmatic LLM-based version.
