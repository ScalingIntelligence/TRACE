...
Reward: DB hash match (1.0), near-miss (0.1 if all-but-one tool names correct), failure (0.0).
"""
    ...
    if scenario_domain == "retail":
        ...
        for op in operations:
            if op.tool_name in ("update_reservation_flights",
                ...
                # Extract date from flight args
                date = (flight_args[0].get("date", "")
                        if flight_args
                        else "")
                # Fallback: try to find date from the DB flights
                if not date and flight_args:
                    fnum = flight_args[0].get("flight_number", "")
                    flight_data = db.get("flights", {}).get(fnum, {})
                    dates = list(flight_data.get("dates", {}).keys())
                    if dates:
                        date = dates[0]
                route = (origin, dest, date)
                if origin and dest and date and route not in searched_routes:
...
def _compute_gold_db_hash(domain, initial_db, operations):
    """Replay expected operations on fresh DB copy, return hash."""
    import copy as _copy
    db = _copy.deepcopy(initial_db)
    gold = ToolExecutor(domain, db)
    for op in operations:
        try:
            gold.execute(op.tool_name, op.tool_args)
        except Exception:
            pass
    return get_dict_hash(gold.db)


def _check_communicate(info_list, conversation):
    """Check if assistant messages mention all required info items."""
    if not info_list:
        return True
    text = " ".join(
        msg.get("content", "") or ""
    ...
    return all(str(i).lower().replace(",", "") in text for i in info_list)
...
def compute_reward(
    ...
    """Compute reward based on DB hash comparison.

    Near-binary reward to avoid the partial-credit trap:
      - DB hash match:       1.0  (full success)
      - DB mismatch but all-but-one tool names correct: 0.1  (near miss)
    ...
    No wrong-call penalty: extra calls that don't change DB state are fine in
    tau2-bench. The DB hash comparison naturally catches wrong state.
    """
    n_ops = len(operations)
    if n_ops == 0:
        return 0.0, "No operations expected."

    # --- Primary signal: DB hash comparison ---
    gold_hash = _compute_gold_db_hash(domain, initial_db, operations)
    ...
    if db_match:
        ...
        # Near-miss check: did the agent get all-but-one tool names correct?
        # Count tool-name matches (ignoring args) as a proxy for "close"
        expected_names = [op.tool_name for op in operations]
        LOOKUP_TOOLS = {
        ...
        agent_write_names = [
            ...
            if c.get("name", "") not in LOOKUP_TOOLS
            and c.get("name", "") != "respond_to_user"
        ]
        # Greedy matching: for each expected name, check if agent produced it
        remaining = list(agent_write_names)
        matched_count = 0
        for ename in expected_names:
            if ename in remaining:
                remaining.remove(ename)
                matched_count += 1
        if matched_count >= n_ops - 1 and matched_count > 0:
            db_reward = 0.1
        else:
            db_reward = 0.0
    ...
    comm_pass = True
    if communicate_info:
        comm_pass = _check_communicate(communicate_info, conversation or [])
        if not comm_pass:
            final_reward = db_reward * 0.0  # zero out if communicate fails
        else:
            final_reward = db_reward
    else:
        final_reward = db_reward

    # --- Build reason string ---
    details = []
    details.append(f"DB hash {'MATCH' if db_match else 'MISMATCH'}")
    if communicate_info:
        details.append(f"Communicate {'PASS' if comm_pass else 'FAIL'}")

    reason = f"reward={final_reward:.2f}. " + "; ".join(details)
    ...
    def step(self, action: Optional[str]) -> None:
        ...
        if tool_name in ("respond_to_user", "send_message"):
            reward, reason = compute_reward(
                    self._tool_calls, self._scenario.operations,
                    self._scenario.domain, self._scenario.db,
                    self._tools,
                    communicate_info=self._scenario.communicate_info,
